    ...
    this.receiveAttack = function receiveAttack(attatcker, location, name){
        for(let i = 0; i < this.ships.length; i++){
            if(this.ships[i].position.includes(location)){
                ...
                document.getElementById(`${attatcker}${location}`).style = "background-color:rgb(211, 58, 58)";
                return;
            }
        }
            document.getElementById(`${attatcker}${location}`).style = "background-color:rgb(74, 133, 62)";
